-- // cx.farm Native Key System Loader //
-- Hosted on GitHub, Routes via Junkie SDK
...
-- // DUAL-BUILD ROUTING ENGINE (SMART TAG CHECKER) //
local function LoadMainScript(junkieResult)
    ScreenGui:Destroy()
    
    local isPremium = false
    
    -- Deep scan the Junkie API response for "Premium" tags, roles, or levels
    if type(junkieResult) == "table" then
        -- 1. Check Level Numbers
        local level = tonumber(junkieResult.level) or tonumber(junkieResult.tier) or 1
        if level >= 2 then isPremium = true end
        
        -- 2. Check Tags/Roles as Strings
        local roleStr = tostring(junkieResult.role or junkieResult.tags or junkieResult.group or ""):lower()
        if string.find(roleStr, "premium") then
            isPremium = true
        end
        
        -- 3. Check Tags if it is an array
        if type(junkieResult.tags) == "table" then
            for _, tag in pairs(junkieResult.tags) do
                if tostring(tag):lower():match("premium") then
                    isPremium = true
                end
            end
        end
...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if result and result.valid then
            ...
            -- Detect Premium visually for the UI
            local isPremiumUI = false
            if type(result) == "table" then
                if (tonumber(result.level) or 1) >= 2 or tostring(result.role or result.tags):lower():match("premium") then
                    isPremiumUI = true
                elseif type(result.tags) == "table" then
                    for _, t in pairs(result.tags) do if tostring(t):lower():match("premium") then isPremiumUI = true end end
                end
            end
...
task.spawn(function()
    ...
    if savedKey and savedKey ~= "" then
        ...
        if result and result.valid then
            ...
            if isPremiumUI then
                UpdateStatus("Premium Key Authenticated!", Theme.Success)
            else
